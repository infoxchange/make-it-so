...
export class IxCloudWatchAlarm extends Construct {
    constructor(scope, id, props) {
        ...
        if (props.toNotify && props.alarmDescription?.includes(". notify:")) {
            throw new Error("Alarm description cannot include '. notify:' if using the toNotify property.");
        }
        ...
        const apiGatewayLatencyMetric = new CloudWatch.Metric(normalizedProps.metric);
...
function convertToActionObject(scope, id, actionType, action) {
    if (typeof action === "string") {
        if (action.startsWith("arn:aws:lambda:")) {
            return new CloudWatchActions.LambdaAction(Lambda.Function.fromFunctionArn(scope, `${id}-${actionType}LambdaAction-${action}`, action));
        }
        else if (action.startsWith("arn:aws:sns:")) {
            return new CloudWatchActions.SnsAction(SNS.Topic.fromTopicArn(scope, `${id}-${actionType}SNSAction-${action}`, action));
        }
        else {
...
*/
function setupCloudFrontAlarm(scope, id, props) {
    ...
    const convertActionToArn = (action) => {
        // Create a forward reference for binding actions
        const alarmRef = CloudWatch.Alarm.fromAlarmName(scope, `${id}-AlarmRef`, alarmName);
        if (typeof action === "string") {
            return action;
        }
        else {
    ...
    for (const actionTypeProp of Object.keys(actionTypes)) {
        for (const action of props.actions?.[actionTypeProp] || []) {
            ...
            for (const resourceTypeKey of Object.keys(resourceTypes)) {
                if (!arn) {
                    throw new Error(`Empty ARN given for ${actionTypes[actionTypeProp]} action.`);
                }
                if (arn.startsWith(`arn:aws:${resourceTypeKey}:`) &&
                    !arn.startsWith(`arn:aws:${resourceTypeKey}:${region}:`)) {
                    throw new Error(`${resourceTypes[resourceTypeKey]} for ${actionTypes[actionTypeProp]} action must live in the same region as the alarm, ${region}.`);
                }
            }
        }
    }
    ...
    const alarmParams = {
        ...
        Dimensions: [
            ...Object.entries(props.metric.dimensionsMap || {}).map(([Name, Value]) => ({ Name, Value })),
    ...
    new CdkCustomResources.AwsCustomResource(scope, "CloudFrontAlarm", {
        onCreate: {
            ...
            physicalResourceId: CdkCustomResources.PhysicalResourceId.of(expectedAlarmArn),
        },
        onUpdate: {
            service: "CloudWatch",
            action: "putMetricAlarm",
